...
Yado is a set of self-hosted services under `*.yado.my.id`, all
deployed on the owner's homelab. This repo is the **landing page + status
dashboard**.

| Service | Repo | Status |
|---|---|---|
| **SSO** | `srytmj/sso.yado` | live - OAuth2 identity for every service |
...
│  · normalized read API      │        │    Yado / File / Demo │
└─────────────────────────────┘        └──────────────────────────────┘
...
interface and ships `YadoSource` to consume it. Pore.js also ships
